...
| `cache.py` | Result caching | `ResultCache` |
...
- `agents run <agent_id>`: Execute single agent. `--model`, `--json`, `--timeout`.
- `workflows run`: Execute workflow DAG sequentially.
- `workers start`: Launch HTTP worker node. `--port`.
- `doctor`: Health checks, token validation, API connectivity.
...
- **testing**: pytest + pytest-asyncio. Mocked API calls. 31 unit tests + integration suite.
...
`src/replicate_mcp/plugins/` → plugin ecosystem  
`src/replicate_mcp/utils/` → audit, checkpointing, telemetry, router state  
`tests/unit/` → 31 unit tests (90%+ coverage required)  
`tests/integration/` → integration tests  
`tests/load/` → load tests (locustfile.py)  
`examples/` → workflow examples, notebooks  
`docs/` → ADRs, guides, runbooks, API reference  
...
- `poetry run mypy src/` — strict type check (33 modules)
- `poetry run ruff check .` — lint (E W F I N UP ASYNC S rules)
- `poetry run ruff format .` — format

envFiles:
- `.env` (not tracked; optional for REPLICATE_API_TOKEN)
...
ci: `.github/workflows/security.yml` → security scanning  
...
|-------|------|---------|---------|---------|
| root | `src/replicate_mcp/__init__.py` | public API surface | new feature discovery | external consumers |
| root | `pyproject.toml` | build config, deps, tool configs | dependency updates, version | all builds |
...
| cli | `src/replicate_mcp/cli/main.py` | CLI commands | command implementation | user workflows |
...
- **unit tests**: `tests/unit/test_*.py` (31 files)
...
**Roadmap**:
- Phase 6.5: Cost estimation (`--dry-run`) + doctor command enhancements
- Phase 7: Workflow versioning + rollback support
- Phase 8: Cost analytics dashboard + multi-tenant isolation
...
- `replicate-agent doctor` — health checks
...
**Generated 2026-04-27** | `replicate-mcp-agents` v0.6.0 | 370 lines
